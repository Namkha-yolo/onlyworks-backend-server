-- Migration 002: Update all foreign keys to reference users(id) instead of profiles(id)
-- Description: Point all user_id foreign keys to the users table

-- WARNING: This migration requires that:
-- 1. Data has been migrated from profiles to users (migration 003)
-- 2. No orphaned records exist in child tables
